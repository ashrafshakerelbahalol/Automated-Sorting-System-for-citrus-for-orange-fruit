...
def automtic_oranges(image_path):
    size_list = []
    color_list = []
    ...
    for contour in hull_contours:
        ...
        # cv.putText(image_copy, "R=%d" % radius, (int(cx-radius), int(cy-radius)), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        ...
        if circularity > circularity_threshold or ellipticity > ellipticity_threshold:
            ...
            # Append the valid CCI value to the result list
            size_list.append(radius)
            color_list.append(CCI)
